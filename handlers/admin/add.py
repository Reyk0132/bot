...
async def process_settings(message: Message):
    ...
    markup.add(InlineKeyboardButton(
        '+ Добавить качкозавра', callback_data='add_category'))
    
    await message.answer('Качкозавры', reply_markup=markup)

@dp.callback_query_handler(IsAdmin(), text='add_category')
async def add_category_callback_handler(query: CallbackQuery):
    await query.message.delete()
    await query.message.answer('Имя качкозавра?')
...
add_product = 'Добавить упражнение'
delete_category = 'Продать качкозавра'

async def show_products(m, products, category_idx):
    await bot.send_chat_action(m.chat.id, ChatActions.TYPING)

    for idx, title, body, video, price, tag in products:
        text = f'<b>{title}</b>\n\n{body}\n\nЦена: {price} рублей.'
        ...
        await m.answer_photo(photo=video, 
                             caption=text, 
                             reply_markup=markup)
    ...
    await m.answer('Хотите добавить упражнение?',
                   reply_markup=markup)
...
async def delete_category_handler(message: Message, state: FSMContext):
    async with state.proxy() as data:
        if 'category_index' in data.keys():
            ...
            await message.answer('Продан!', reply_markup=ReplyKeyboardRemove())
...
async def process_body(message: Message, state: FSMContext):
    ...
    await ProductState.next()
    await message.answer('Изображение/видео?', reply_markup=back_markup())

@dp.message_handler(IsAdmin(), content_types=ContentType.VIDEO, state=ProductState.video)
...
async def process_price(message: Message, state:FSMContext):
    async with state.proxy() as data:
        ...
        await message.answer_photo(photo=data['video'], caption=text, reply_markup=markup)
...
async def process_confirm(message: Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        db.query('INSERT INTO products VALUES (?, ?, ?, ?, ?, ?)',
                 (idx, title, body, video, int(price), tag))
...
@dp.message_handler(IsAdmin(), content_types=ContentType.TEXT, state=ProductState.video)
async def process_image_url(message: Message, state: FSMContext):
    if message.text == back_message:
        ...
        await message.answer('Вам нужно прислать видео.')

@dp.message_handler(IsAdmin(), lambda message: not message.text.isdigit(), state=ProductState.price)
async def process_price_invalid(message: Message, state: FSMContext):
    if message.text == back_message:

        await ProductState.video.set()

        async with state.proxy() as data:
            
            await message.answer("Другое видео?", reply_markup=back_markup())
...
async def process_confirm_invalid(message: Message, state: FSMContext):
    await message.answer('Такого товара не было.')